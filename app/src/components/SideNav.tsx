...
export function SideNav({ email }: { email: string }) {
  ...
  const items = [
    { href: '/dashboard', label: 'Contacts', icon: ICONS.home, active: path === '/dashboard' || path.startsWith('/dashboard/leads') },
    { href: '/dashboard/sequence', label: 'Séquence', icon: ICONS.seq, active: path.startsWith('/dashboard/sequence') },
    { href: '/dashboard/settings', label: 'Réglages', icon: ICONS.cog, active: path.startsWith('/dashboard/settings') },
  ];
